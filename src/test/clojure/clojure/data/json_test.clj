...
(defn pbr
  ...
  ([s size]
   (if (< size 64)
     (throw (Exception. "Size must be >= 64"))                                  ;;; RuntimeException.
     (clojure.lang.PushbackTextReader.(System.IO.StringReader. s) #_size ))))   ;;; java.io.PushbackReader. java.io.StringReader.  -- removed size parameter -- we don't support it
...
(deftest lenient-on-extra-data
  ...
  (is (= [42] (json/read (System.IO.StringReader. "[42],abc")))))                                          ;;; java.io.StringReader.
...
(deftest strict-on-extra-data
  ...
  (is (thrown? clojure.lang.ExceptionInfo
        (json/read (System.IO.StringReader. "[42],abc") :extra-data-fn json/on-extra-throw)))              ;;; java.io.StringReader.
  ...
  (is (= :hi (json/read (System.IO.StringReader. "")                                                       ;;; java.io.StringReader.
               :eof-error? false, :eof-value :hi, :extra-data-fn json/on-extra-throw))))
...
(deftest read-nested-structures-stream
  (is (= {:a [1 2 {:b [3 "four"]} 5.5]}
         (json/read (System.IO.StringReader. "{\"a\":[1,2,{\"b\":[3,\"four\"]},5.5]}")     ;;; java.io.StringReader.
                    :key-fn keyword))))
...
(deftest convert-values
  (is (= {:number 42 :date (System.DateTime. 1955 6 12)}                                  ;;; (java.sql.Date. 55 6 12)
...
(deftest print-uuids
  (let [uid (System.Guid.)                                                          ;;; java.util.UUID/randomUUID
        roundtripped  (System.Guid. (json/read-str (json/write-str uid)))]          ;;; java.util.UUID/fromString
    (is (= uid roundtripped))))
...
(deftest error-on-NaN
  (is (thrown? Exception (json/write-str Single/NaN)))                            ;;; Float/NaN
  (is (thrown? Exception (json/write-str Double/NaN))))

(deftest error-on-infinity
  (is (thrown? Exception (json/write-str Single/PositiveInfinity)))               ;;; Float/POSITIVE_INFINITY
  (is (thrown? Exception (json/write-str Single/NegativeInfinity)))               ;;; Float/NEGATIVE_INFINITY
  (is (thrown? Exception (json/write-str Double/PositiveInfinity)))               ;;; Double/POSITIVE_INFINITY
  (is (thrown? Exception (json/write-str Double/NegativeInfinity))))              ;;; Double/NEGATIVE_INFINITY
...
(deftest special-handler-for-double-infinity
  (is (= "{\"double\":\"Infinity\"}"
         (json/write-str {:double Double/PositiveInfinity}                      ;;; POSITIVE_INFINITY
                         :value-fn double-value)))
  (is (= "{\"double\":\"-Infinity\"}"
         (json/write-str {:double Double/NegativeInfinity}                      ;;;NEGATIVE_INFINITY
                         :value-fn double-value))))
...
(deftest default-throws-on-eof
  (is (thrown? System.IO.EndOfStreamException (json/read-str ""))))                      ;;; java.io.EOFException
